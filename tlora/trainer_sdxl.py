...
class LoraTrainerSDXL:
    ...
    def setup_exp_name(self, exp_idx):
        exp_name = "{0:0>5d}-{1}-{2}".format( #00001-a3f4-dataset
            exp_idx + 1,
            secrets.token_hex(2),
    ...
    def setup_model(self):
        ...
        self.unet.set_attn_processor(lora_attn_procs) # re-apply newly constructed processors back to attention blocks
    ...
    def train_step(self, batch):
        # Get VAE latents
        if self.config.with_prior_preservation: #DreamBooth dataset 
            latents = self.vae.encode(
                batch["pixel_values"].to(self.weight_dtype)
            ).latent_dist.sample()
        else:
            latents = self.vae.encode(
                batch["image"].to(self.weight_dtype) * 2.0 - 1.0 # [0, 1] scale to [-1, 1]
            ).latent_dist.sample()

        # Scale the latents by the vae scaling factor
        ...
        # Get text latents for conditioning UNets
        if not self.config.with_prior_preservation: #normal dataset 
        ...
        # output through UNet, get the predicted noise
        outputs = self.unet(
            noisy_latents,
            timesteps,
        ...
        # calculate loss
        # if with_prior_preservation, the batch is a concatenation of instance images and class images, and the output and target are chunked into two parts for separate loss calculation. The final loss is a weighted sum of instance loss and prior loss.
        if self.config.with_prior_preservation:
    ...
    def train(self):
        ...
        # final validation and saving after training
        if self.accelerator.is_main_process:
            if self.config.wandb_api_key is not None and self.config.validation_prompts:
